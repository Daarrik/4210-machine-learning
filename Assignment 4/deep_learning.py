...
# SPECIFICATION: Deep learning to classify fashion items from tf.keras.datasets.fashion_mnist
...
for h in n_hidden:                          #looking or the best parameters w.r.t the number of hidden layers
    for n in n_neurons:                     #looking or the best parameters w.r.t the number of neurons
        for l in l_rate:                    #looking or the best parameters w.r.t the learning rate
            ...
            # Other ways to compute accuracy:
            # Using keras:
            # test_loss, test_acc = model.evaluate(X_test, y_test, verbose=0)
            # test_acc is the accuracy
            # OR
            # Iterating through 
            # num_correct = 0
            # for prediction, true_val in zip(class_predicted, y_test):
            #     if prediction == true_val:
            #         num_correct += 1
            # for_acc = num_correct / len(y_test)
            # print(test_acc)
            # print(accuracy)
            # print(for_acc)
